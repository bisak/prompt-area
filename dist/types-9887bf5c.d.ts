...
type TriggerConfig = {
    ...
    /**
     * For 'callback' mode: when true, the trigger character and any typed query
     * are stripped from the editor before `onActivate` fires, no chip is
     * inserted, and no dropdown is shown — the editor is left exactly as it was
     * before the trigger character was typed. Use for triggers that open their
     * own surface (command palette, file picker, side menu). Ignored in
     * 'dropdown' mode.
     */
    silent?: boolean;
    /**
